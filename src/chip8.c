...
void chip8_exec(struct chip8 *chip8, unsigned short opcode){
    ...
    switch (opcode & 0xf000){
        ...
        {
            unsigned char v_i = (opcode & 0x0f00) >> 8; 
            unsigned char comp_value = (opcode & 0x00ff);
        ...
        case 0x6000: //0x6xkk set vx = kk
            {
            unsigned char v_i = (0x0f00 & opcode) >> 8; 
        ...
        case 0xC000: //0xCxkk Vx = random byte(0 to 255) AND kk
            {
            unsigned char v_i = (opcode & 0x0f00) >> 8; 
            unsigned char val = rand()%256 & (opcode & 0x00ff);
        ...
        case 0xD000: 
            ...
            unsigned char xpos = chip8->registers.V[x];
            unsigned char ypos = chip8->registers.V[y];
            unsigned char *SpritePtr = &chip8->memory.memory[chip8->registers.I];
            unsigned char size = (opcode & 0x000f);
    ...
    switch(opcode & 0xf00f){
        ...
        case 0x8005: //0x8xy5 Vx= Vx-Vy, VF = !borrow of dif
            { 
            short dif = chip8->registers.V[x] - chip8->registers.V[y];
            chip8->registers.V[0xf] = 0;
            if(dif > 0){
                chip8->registers.V[0xf] = 1;
            }
        ...
        case 0x8007: //0x8xy7 Vx = Vy-Vx, VF = !borrow of dif
            { 
            short dif = chip8->registers.V[y] - chip8->registers.V[x];
            chip8->registers.V[0xf] = 0;
            if(dif > 0){
                chip8->registers.V[0xf] = 1;
            }
    ...
    switch(opcode & 0xf0ff){
        ...
        case 0xF055: //store registers V0 through Vx in memory
            ...
            {
            short address = chip8->registers.I;
            for(int i = 0; i <= x; i++){
